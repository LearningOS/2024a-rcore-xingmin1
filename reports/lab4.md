...
> 1. 使用pipe
     ...
     >     可以使用一个应用专门中转消息。
     >
     >     * 需要互相通信多个进程都与这个中转应用建立一个管道。
>     * 中转应用给这些进程同步一下标识相关的信息
>     * 在发送消息时，发送方要给出接收方的标识
>     * 由中转应用进行转发，在转发时，中转应用要附上发送方的标识
> 2. 扩展pipe，将pipe的口多开几个。
     >
     >     * 在发送消息时，发送方要给出双方的标识
>     * 每个应用都可以通过pipe查看其他同样持有pipe的应用的标识相关的信息
>     * 每个应用自己搭建滤网，或者在建立管道时自动搭建滤网，只接受发送给自己的消息
> 3. 使用共享内存以及访问共享内存所需用到的同步互斥机制
> 4. 使用消息队列
> 5. 使用发布-订阅模式
